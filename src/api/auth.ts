  ...
  try {
    ...
    if (error instanceof ApiError) {
      if (error.isUnauthorized || error.status === 403) {
        throw createErrorWithCause('Invalid email or password. Please try again.', error);
      }

      if (error.status === 429) {
        throw createErrorWithCause('Too many login attempts. Please try again later.', error);
      }
    ...
    throw createErrorWithCause('Login failed. Please try again.', error);
  }
}
...
export async function updateUser(id: string, data: UpdateUserRequest): Promise<User> {
  try {
    ...
    throw createErrorWithCause('Update failed. Please try again.', error);
  }
}
...
export async function register(credentials: RegisterRequest): Promise<void> {
  try {
    ...
    // Generic error fallback
    throw createErrorWithCause('Registration failed. Please try again.', error);
  }
}
