...
export async function requestPasswordReset(email: string): Promise<PasswordResetResponse> {
  try {
    ...
    if (error instanceof ApiError) {
      ...
      if (error.status >= 500) {
        throw createErrorWithCause('Server error. Please try again later.', error);
      }
    ...
    throw createErrorWithCause('Password reset request failed. Please try again.', error);
  }
}
...
export async function validatePasswordResetToken(
  ...
  try {
    ...
    if (error instanceof ApiError) {
      if (error.status === 400) {
        throw createErrorWithCause('Invalid or expired reset token.', error);
      }
    ...
    throw createErrorWithCause('Token validation failed. Please try again.', error);
  }
}
...
export async function confirmPasswordReset(token: string, newPassword: string): Promise<void> {
  try {
    ...
    if (error instanceof ApiError) {
      ...
      if (error.status === 429) {
        throw createErrorWithCause('Too many attempts. Please try again later.', error);
      }
    ...
    throw createErrorWithCause('Password reset failed. Please try again.', error);
  }
}
